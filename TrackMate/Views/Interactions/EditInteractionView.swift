...
struct EditInteractionView: View {
   @ObservedObject var interaction: Interaction
	...
	@State private var overallExperience: String
    ...
    init(interaction: Interaction) {
	    ...
	    _overallExperience 			= State(wrappedValue: interaction.overallExperience ?? "I'm Not Sure")
    }
    ...
    var body: some View {
        NavigationStack {
		   Form {
			   ...
			   Text("Overall, would you say this was a positive or negative interaction?")
				   .foregroundColor(themeManager.color("SecondaryText"))
			   
			   Picker("Overall experience", selection: $overallExperience) {
				   ForEach(overallOptions, id: \.self) { Text($0) }
			   }
			   .pickerStyle(SegmentedPickerStyle())
			   .foregroundColor(themeManager.color("SecondaryText"))
			   .padding(.bottom)
